...
- Endpoints: `GET/POST /api/track/reference`, `GET /api/admin/heatmap/reference`; site-config now carries `clarity_id`.
- Self-verified: endpoints validated via curl (allowlist enforcement, upload/fetch), reference auto-capture confirmed working, login page renders with no console/page errors, clean compile.
...
## Implemented (2026-07-31, part 2) — Auto incidents + Subscribers + Uptime windows (Status Page)
- **Auto Incident Post**: `_sync_public_incidents` runs inside every diagnostics scan (auto re-scan + manual) — auto-opens a public incident (`status_incidents`) when a subsystem group breaks (degraded/outage) and auto-resolves it when it recovers. Gated by an admin `auto_incidents` toggle (default on). Admins can also manually post incidents/updates/resolve.
- **Email Subscribers**: public `POST /api/status/subscribe` (email validation, dedup, 404 when page disabled) + `GET /api/status/unsubscribe?token=`. `_notify_subscribers` emails all active subscribers (best-effort) whenever an incident is opened/updated/resolved — includes an unsubscribe link. Subscribe box lives on `/status`. (Live delivery needs a configured email provider; fails gracefully otherwise.)
...
- **Ops Overview auto-refresh** (part 12b): the widget now re-fetches `ops-overview` every 30s (interval cleared on unmount; transient errors keep last-known data) with a subtle green "live" pulse indicator (`ops-live-indicator`) in the header. Self-certified: build clean, display-only change to the iteration_55-verified widget.
- **Ops Overview manual rescan** (part 12c): a "Run diagnostics now" button (`ops-rescan-btn`) on the Ops Overview card POSTs `/admin/deploy/diagnose` (autofix:false), toasts the summary (ok/warn/fail), and reloads the widget. Reuses the verified diagnose endpoint. Curl-verified (summary + fresh generated_at); build clean; self-certified (display + existing-endpoint reuse).
