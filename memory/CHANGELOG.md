...
## Implemented (2026-07-31, part 6) — Recategorize Memory, Memory Digest, Preset Diff On Click
- **Recategorize Memory**: PATCH /api/ai/memory/{id} now accepts an optional `category` (content optional). Each memory row in the panel has a category dropdown to move a fact between Preferences/Projects/Deadlines/Tools/General.
- **Memory Digest**: per-user `memory_digest` opt-in pref (in ai_user_prefs). New POST /api/ai/memory/digest/send-now emails a grouped "What Stitch remembers about you" summary with a one-click prune link (FRONTEND_URL/assistant?memory=open — AiAssistant auto-opens the panel on that param). Monthly background sender `scan_memory_digests()` added to server.py reminder loop (sends to opted-in users, >=30 days since last). Uses the app email pipeline (send_email_detailed) — delivery currently fails gracefully (401/no SMTP app password) until real email creds are set; the HTML build + send path are verified.
- **Preset Diff On Click**: clicking ANY preset (built-in or custom) now opens the same Adds/Removes diff modal before applying; built-in/custom clicks apply-only, import saves + applies (button/title adapt).
- Verified via curl (recategorize=200, digest toggle, send-now attempts+graceful fail) and screenshots (grouped panel with category dropdowns + digest card, preset diff modal, ?memory=open auto-open).

## Implemented (2026-07-31, part 7) — Digest Cadence, Bulk Recategorize
...
## Implemented (2026-07-31, part 11) — Auto Re-scan & alerts, Import Memories
- **Auto Re-scan**: background scan_auto_diagnostics() (in server.py reminder loop) re-runs diagnostics on schedule when enabled (settings diagnostics_auto), compares to the previous report, and records an alert for any check that regressed (ok→warn/fail); emails admins (graceful). Endpoints: GET /admin/deploy/diagnose/state, PUT /admin/deploy/diagnose/auto {enabled}, POST /admin/deploy/diagnose/alerts/seen. UI: an "Auto on/off" toggle + a red "N new issues detected since last scan" banner (with per-check from→to and dismiss) in the diagnostics card. Regression detection verified directly (broke TURN → alert created).
- **Import Memories**: new POST /api/ai/memory/import {memories:[...]} re-inserts a previously exported set with fresh IDs (dedup by content, respects max_items). UI: an Import button + hidden file input in the Memory panel that parses .json or .csv client-side and uploads.
- Verified via curl (import=2), direct async regression test (2 alerts created + graceful admin email), and screenshots (import button + grouped imported memories; auto toggle + alerts banner).

## Implemented (2026-07-31, part 12) — Scan History, Alert Channels
