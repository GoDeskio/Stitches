...
import { Tag, Plus, Pencil, Trash2, Star, ExternalLink, Lock } from "lucide-react";
...
const EMPTY = { name: "", description: "", price: "", yearly_price: "", interval: "month", featuresText: "", feature_keys: [...ALL_KEYS], highlighted: false, cta: "Get started", sort_order: 0, active: true };
...
export function PlansTab() {
  ...
  useEffect(() => {
    load();
    api.get("/admin/plan-gating").then(({ data }) => setGating(!!data.enabled)).catch(() => {});
  }, []);

  const toggleGating = async () => {
    const next = !gating;
    setGating(next);
    try { await api.post("/admin/plan-gating", { enabled: next }); toast.success(`Feature gating ${next ? "enabled" : "disabled"}`); }
    catch { setGating(!next); toast.error("Failed to update"); }
  };
  const toggleActive = async (p) => {
    await api.put(`/admin/plans/${p.plan_id}`, { ...p, features: p.features, feature_keys: p.feature_keys, active: !p.active });
    load();
  };
  const del = async (p) => {
  ...
  return (
    <div className="space-y-6" data-testid="plans-tab">
      {/* Gating toggle */}
      <div className="neu-raised rounded-[1.5rem] p-5 flex items-center justify-between gap-4 animate-fade-up" data-testid="plans-gating-card">
        <div className="flex items-center gap-3">
          <div className="neu-sm w-11 h-11 rounded-2xl flex items-center justify-center"><Lock className="w-5 h-5 text-primary-stitch" /></div>
          <div>
            <p className="font-head font-bold text-lg" style={{ color: "var(--text)" }}>Plan-based feature gating</p>
            <p className="text-sm text-muted-stitch">When ON, members only access the app features included in their plan. Admins always have full access.</p>
          </div>
        </div>
        <button data-testid="plans-gating-toggle" onClick={toggleGating}
          className={`w-14 h-8 rounded-full flex items-center px-1 transition-all shrink-0 ${gating ? "justify-end" : "justify-start"}`}
          style={{ background: gating ? "var(--primary)" : "var(--neu-dark)" }}>
              ...
              <div key={p.plan_id} data-testid="plan-card" className={`neu-pressed rounded-2xl p-5 ${p.active ? "" : "opacity-55"}`}>
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0">
                    ...
                    {Number(p.yearly_price) > 0 && <p className="text-xs text-muted-stitch">or {fmtMoney(p.yearly_price)}/yr</p>}
                  </div>
                  <div className="flex gap-1 shrink-0">
                    <button data-testid="plan-edit-btn" onClick={() => setEditing({ ...p, featuresText: (p.features || []).join("\n"), feature_keys: Array.isArray(p.feature_keys) ? p.feature_keys : [...ALL_KEYS] })} className="neu-btn rounded-xl px-2.5 py-2 text-primary-stitch"><Pencil className="w-4 h-4" /></button>
                ...
                <p className="text-[11px] text-muted-stitch mt-2">Unlocks: {Array.isArray(p.feature_keys) ? (p.feature_keys.length === 0 ? "none" : p.feature_keys.length + " feature(s)") : "all features"}</p>
...
function PlanEditor({ plan, onClose, onSaved }) {
  ...
  const toggleFeature = (key) => setF((p) => {
    const has = (p.feature_keys || []).includes(key);
    return { ...p, feature_keys: has ? p.feature_keys.filter((k) => k !== key) : [...(p.feature_keys || []), key] };
  });
  ...
  const save = async () => {
    ...
    setSaving(true);
    const body = {
      name: f.name, description: f.description, price: parseFloat(f.price) || 0,
      yearly_price: parseFloat(f.yearly_price) || 0, interval: f.interval,
      features: (f.featuresText || "").split("\n").map((x) => x.trim()).filter(Boolean),
      feature_keys: f.feature_keys || [],
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" onClick={onClose} data-testid="plan-editor-modal">
      <div className="neu-raised rounded-[1.75rem] p-6 max-w-lg w-full max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="space-y-3">
          ...
          <div className="flex gap-3">
            <div className="flex-1">
              <label className="text-xs font-semibold text-muted-stitch">Monthly price</label>
            ...
            <div className="flex-1">
              <label className="text-xs font-semibold text-muted-stitch">Yearly price</label>
              <input data-testid="plan-yearly" type="number" step="0.01" min="0" value={f.yearly_price} onChange={(e) => set("yearly_price", e.target.value)} placeholder="optional" className="neu-input w-full rounded-2xl py-3 px-4 text-sm mt-1" />
            </div>
            <div className="flex-1">
          ...
          <div>
            <label className="text-xs font-semibold text-muted-stitch">Marketing features (one per line, shown on pricing page)</label>
            <textarea data-testid="plan-features" value={f.featuresText} onChange={(e) => set("featuresText", e.target.value)} rows={4} placeholder={"Unlimited projects\nPriority support"} className="neu-input w-full rounded-2xl py-3 px-4 text-sm mt-1 resize-none" />
          </div>
          <div>
            <label className="text-xs font-semibold text-muted-stitch">App features this plan unlocks (used when gating is ON)</label>
            <div className="grid grid-cols-2 gap-2 mt-2" data-testid="plan-feature-keys">
              {FEATURES.map((ft) => {
                const on = (f.feature_keys || []).includes(ft.key);
                return (
                  <button key={ft.key} type="button" data-testid={`plan-feature-${ft.key}`} onClick={() => toggleFeature(ft.key)}
                    className={`neu-btn rounded-xl px-3 py-2.5 text-sm text-left flex items-center gap-2 ${on ? "text-primary-stitch" : "text-muted-stitch opacity-70"}`}>
                    <span className={`w-4 h-4 rounded flex items-center justify-center text-[10px] ${on ? "text-white" : ""}`} style={{ background: on ? "var(--primary)" : "var(--neu-dark)" }}>{on ? "✓" : ""}</span>
                    {ft.label}
                  </button>
                );
              })}
